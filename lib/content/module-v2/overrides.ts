...
export const MODULE_V2_OVERRIDES: Record<string, ModuleV2Override> = {
  ...
  "proverka-meta-tegov-online": {
    eyebrow: "Title и description",
    headline: "Мета-теги по URL — и что изменилось с прошлой проверки",
    cta: "Проверить мета-теги",
    painGain: {
      painTitle: "Без снимка разметки",
      pains: [
        "Title и description смотрите вручную на сотнях URL",
        "Правки на сайте замечаете после просадки CTR",
        "Нет сравнения «было / стало» по canonical и H1",
      ],
      gainTitle: "В Титло",
      gains: [
        "До 500 URL в одном прогоне — title, description, canonical, H1…",
        "История проверок и сравнение «было / стало»",
        "Уведомления, когда разметка на сайте изменилась",
      ],
    },
    acts: [
      act(
        "01",
        "Вставьте список URL",
        "Свой сайт или конкуренты — до 500 адресов. Отметьте поля: title, description, canonical, H1…",
        META_MON_SCREENSHOTS[0].src,
        META_MON_SCREENSHOTS[0].caption,
        ["URL построчно", "Поля head", "Таймаут"]
      ),
      act(
        "02",
        "Смотрите снимок",
        "По каждому URL — содержимое тегов, счётчики и ошибки. Фильтр по типу проблемы.",
        META_MON_SCREENSHOTS[1].src,
        META_MON_SCREENSHOTS[1].caption,
        ["Title · description", "Canonical · H1", "Ошибки"]
      ),
      act(
        "03",
        "Сравните с прошлым",
        "Две проверки рядом: что изменилось в title, description, canonical. Экспорт при необходимости.",
        META_MON_SCREENSHOTS[2].src,
        META_MON_SCREENSHOTS[2].caption,
        ["Было / стало", "Фильтр изменений", "Экспорт"]
      ),
    ],
    metrics: [
      m("500", "URL", "в одной проверке"),
      m("6+", "полей", "title · H1 · canonical…"),
      m("Δ", "снимки", "сравнение проверок"),
      m("1", "сутки", "автопроверка в кабинете"),
    ],
    storySection: {
      eyebrow: "Как работает",
      title: "От списка URL до сравнения",
      lead: "Список страниц → снимок тегов → сравнение с прошлой проверкой, если разметка изменилась.",
      midCta: {
        title: "Нужны проекты, автоснимки и уведомления?",
        lead: "В демо — одна страница. В кабинете: до 500 URL, история, сравнение и оповещения при изменениях.",
      },
    },
    orbitSection: {
      eyebrow: "Рядом в кабинете",
      title: "С чем стыкуется мониторинг мета-тегов",
      lead: "После снимка разметки — релевантность, конкуренты, позиции и HTTP-заголовки.",
    },
    footerUi: {
      finalTitle: "Снять мета-теги в кабинете",
