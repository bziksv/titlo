...
import {
  PHRASE_COMMERCE_AUDIENCE,
  PHRASE_COMMERCE_FAQ,
  PHRASE_COMMERCE_HERO,
  PHRASE_COMMERCE_PIPELINE,
  PHRASE_COMMERCE_SCENARIOS,
  PHRASE_COMMERCE_STATS,
  PHRASE_COMMERCE_STEPS,
  PHRASE_COMMERCE_TARIFF_LIMITS,
  PHRASE_COMMERCE_WHY,
...
export function GeoLokalizaciyaKommerciyaLanding({ module }: Props) {
  return (
    <>
        ...
        <div className="relative z-10 mx-auto max-w-6xl px-4 py-12 md:py-16">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="rounded-full bg-white/15 px-3 py-1 text-xs font-semibold uppercase tracking-wide">
              {PHRASE_COMMERCE_HERO.badge}
            </span>
          </div>

          <h1 className="mt-5 max-w-4xl text-3xl font-bold leading-tight md:text-4xl">
            {PHRASE_COMMERCE_HERO.title}
          </h1>
          <p className="mt-4 max-w-3xl text-lg text-brand-50/95">{PHRASE_COMMERCE_HERO.lead}</p>
          <p className="mt-2 font-mono text-sm text-white/70">{PHRASE_COMMERCE_HERO.sub}</p>
          ...
          <div className="mt-10">
            <ModuleLeadCta
              variant="hero"
              idPrefix="phrase-commerce-hero"
              moduleSlug="geo-lokalizaciya-kommerciya"
              title="Оценить фразы в кабинете"
              hint="Регистрация — список фраз, обе ПС, выбор регионов, история и выгрузка."
            />
          </div>
      ...
      <DemoShowcaseSection
        ...
        lead="1 фраза · 1 ПС · глубина 10 · до 2 запусков в сутки. Список фраз, регионы и история — в кабинете."
      >
      ...
      <section className="py-14 md:py-16">
        <div className="mx-auto max-w-6xl px-4">
          <h2 className="text-2xl font-bold text-slate-900 md:text-3xl">{PHRASE_COMMERCE_WHY.title}</h2>
          <p className="mt-3 max-w-3xl text-slate-600">{PHRASE_COMMERCE_WHY.lead}</p>
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            {PHRASE_COMMERCE_WHY.items.map((item) => (
      ...
      <section className="border-y border-slate-200 bg-slate-50 py-14 md:py-16">
        <div className="mx-auto max-w-6xl px-4">
          <h2 className="text-2xl font-bold text-slate-900 md:text-3xl">{PHRASE_COMMERCE_AUDIENCE.title}</h2>
          <p className="mt-3 max-w-3xl text-slate-600">{PHRASE_COMMERCE_AUDIENCE.lead}</p>
          <div className="mt-8 grid gap-4 md:grid-cols-2">
            {PHRASE_COMMERCE_AUDIENCE.items.map((item) => (
      ...
      <section className="py-14 md:py-16">
        <div className="mx-auto max-w-6xl px-4">
          <h2 className="text-2xl font-bold text-slate-900 md:text-3xl">{PHRASE_COMMERCE_SCENARIOS.title}</h2>
          <ol className="mt-8 grid gap-4 sm:grid-cols-2">
            {PHRASE_COMMERCE_SCENARIOS.items.map((item, i) => (
      ...
      <section className="border-y border-slate-200 bg-slate-50 py-14 md:py-16">
        <div className="mx-auto max-w-6xl px-4">
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {PHRASE_COMMERCE_STEPS.map((step) => (
      ...
      <section className="py-14 md:py-16">
        <div className="mx-auto max-w-6xl px-4">
          <h2 className="text-2xl font-bold text-slate-900 md:text-3xl">{PHRASE_COMMERCE_PIPELINE.title}</h2>
          <p className="mt-3 max-w-3xl text-slate-600">{PHRASE_COMMERCE_PIPELINE.lead}</p>
          <ul className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {PHRASE_COMMERCE_PIPELINE.links.map((link) => (
              <li key={link.href}>
                <Link
                  href={link.href}
      ...
      <section className="py-12">
        <div className="mx-auto max-w-6xl px-4">
          ...
          <p className="mt-2 text-slate-600">
            Проверки / сохранения. Списание — за запросы к выдаче в двух регионах на фразу.
          </p>
          <div className="mt-6 overflow-hidden rounded-xl border border-slate-200">
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {PHRASE_COMMERCE_TARIFF_LIMITS.map((row) => (
